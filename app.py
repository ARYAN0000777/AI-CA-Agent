...
div.stButton > button { background: linear-gradient(135deg, #7C6FFF, #5B4FE8) !important; color: white !important; border: none !important; border-radius: 11px !important; padding: 11px 24px !important; font-weight: 600 !important; width: 100%; }
...
with tab1:
    ...
    with col_upload:
        ...
        if uploaded_file is not None and st.session_state.scanned_data is None:
            ...
            st.markdown('<div class="step-row"><div class="step-num">2</div><div class="step-label">Run AI extraction on the invoice</div></div>', unsafe_allow_html=True)
    ...
    if st.session_state.scanned_data is not None:
        ...
        st.markdown('<div class="step-row"><div class="step-num">3</div><div class="step-label">Verify & edit extracted data before saving</div></div>', unsafe_allow_html=True)
...
with tab2:
    ...
    if audio_value is not None and st.session_state.voice_scanned_data is None:
        ...
        if st.button("🚀 Generate Bill from Voice", use_container_width=True):
            with st.spinner("AI is listening to your voice and determining Sales/Purchase..."):
                try:
                    audio_prompt = """
...
with tab3:
    ...
    if total_bills > 0:
        ...
        def create_pdf_bill(bill_data):
            ...
            # THE FIX: Ensuring values are cast to float correctly
