...
html, body, .stApp { background-color: var(--bg) !important; color: var(--text) !important; font-family: 'DM Sans', sans-serif !important; -webkit-font-smoothing: antialiased !important; -moz-osx-font-smoothing: grayscale !important; }
...
.export-icon { font-size: 2.4rem; display: block; margin-bottom: 0.6rem; }
.export-title { font-family: 'Syne', sans-serif; font-size: 1.1rem; font-weight: 700; color: #FFFFFF; margin-bottom: 0.4rem; }
.export-desc { font-size: 0.82rem; color: var(--muted-2); max-width: 320px; margin: 0 auto; }
...
try:
    ...
    if not all([AI_API_KEY, GROQ_API_KEY, SUPABASE_URL, SUPABASE_KEY]):
...
try:
    ...
    st.warning(f"⚠️ Database connection error: {str(e)}")
    db_data = []
                    ...
                    try:
                        ...
                        for attempt in range(max_retries):
                            try:
                                ...
                                if "503" in str(api_e) or "high demand" in error_str or "429" in str(api_e):
                                    if attempt < max_retries - 1:
                                        st.warning(f"⏳ Server traffic high. Auto-retrying in 3s... (Attempt {attempt + 1}/{max_retries})")
                                        time.sleep(3)
                                    else:
                                        st.error("❌ Servers overloaded. Try again later.")
                                else:
...
with tab2:
    ...
    st.info("💡 Pro Tip: Speak naturally about the transaction. System will auto-detect Purchase or Sales.")
            ...
            with st.spinner("Running acoustic models and calculating GST..."):
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if "503" in str(e) or "high demand" in error_str or "429" in str(e):
                            if attempt < max_retries - 1:
                                st.warning(f"⏳ Server busy. Auto-retrying... (Attempt {attempt + 1}/{max_retries})")
                                time.sleep(3)
                            else:
                                st.error("❌ Servers overloaded. Try again later.")
                        else:
    ...
    if st.session_state.voice_scanned_data is not None:
        ...
        if "fetched_name" not in st.session_state: 
            st.session_state.fetched_name = v_data.get("vendor_name", "")
        if "fetched_address" not in st.session_state: 
            st.session_state.fetched_address = v_data.get("vendor_address", "")
            ...
            with st.spinner("Fetching from Server..."):
                try:
                    ...
                    if voice_gst in dummy_db:
                        ...
                        st.warning("⚠️ GST not in local database. Please enter manually.")
                except Exception as e:
        ...
        with st.form("edit_voice_form"):
            ...
            with c1: 
                voice_vendor = st.text_input("Counterparty", value=v_data.get("vendor_name", "Local Party"))
            with c2: 
                voice_date = st.text_input("Timestamp", value="2026-04-09")

            r1, r2 = st.columns(2)
            with r1: 
                final_vendor_name = st.text_input("Legal Name", value=st.session_state.fetched_name)
            with r2: 
                final_address = st.text_input("Registered Address", value=st.session_state.fetched_address)
...
with tab3:
    ...
    with m1: 
        st.markdown(f'<div class="metric-card purple"><span class="metric-icon">📄</span><div class="metric-label">Ledger Entries</div><div class="metric-value purple">{total_bills}</div></div>', unsafe_allow_html=True)
    with m2: 
        st.markdown(f'<div class="metric-card green"><span class="metric-icon">📈</span><div class="metric-label">Gross Revenue</div><div class="metric-value green">₹{total_sales:,.0f}</div></div>', unsafe_allow_html=True)
    with m3: 
        st.markdown(f'<div class="metric-card amber"><span class="metric-icon">📉</span><div class="metric-label">Total Outflow</div><div class="metric-value amber">₹{total_purch:,.0f}</div></div>', unsafe_allow_html=True)
    ...
    if total_bills > 0:
        ...
        if 'voucher_type' not in display_df.columns: 
            display_df['voucher_type'] = 'Purchase'
...
with tab4:
    ...
    with ec1: 
        st.markdown("""<div class="metric-card purple center"><span class="metric-icon">🏦</span><div class="metric-label">Target Format</div><div style="color:#A89EFF;font-family:'Syne',sans-serif;font-weight:800;font-size:1.1rem;">Tally ERP 9</div></div>""", unsafe_allow_html=True)
    with ec2: 
        st.markdown(f"""<div class="metric-card green center"><span class="metric-icon">📦</span><div class="metric-label">Compiled Batch</div><div style="color:#6EE7B7;font-family:'Syne',sans-serif;font-weight:800;font-size:1.1rem;">{len(selected_invoices)} Records</div></div>""", unsafe_allow_html=True)
    with ec3: 
        st.markdown("""<div class="metric-card amber center"><span class="metric-icon">⚡</span><div class="metric-label">System Status</div><div style="color:#FCD34D;font-family:'Syne',sans-serif;font-weight:800;font-size:1.1rem;">Ready for Link</div></div>""", unsafe_allow_html=True)
    ...
    if len(selected_invoices) > 0:
        ...
        st.download_button(
            label="📥 Initialize Download (KhataAI_ERP.xml)", 
            data=generate_tally_xml(selected_invoices), 
            file_name="KhataAI_ERP_Import.xml", 
            mime="application/xml", 
            use_container_width=True
        )
    else:
        st.info("💡 Add invoices first to export to Tally!")
        ...
        with st.chat_message("assistant"):
            with st.spinner("CA Sahab dimaag laga rahe hain... 🧠"):
                try:
                    ...
                    - IMPORTANT: Never start your answer the same way. Be fresh and creative!
                    """
                    ...
                    try:
                        ...
                        tts.write_to_fp(af)
                        af.seek(0)
                        st.audio(af, format='audio/mp3', autoplay=False)
                    except Exception as tts_e:
                        st.warning(f"⚠️ Audio generation skipped: {str(tts_e)}")
                        
                except Exception as e:
                    st.error(f"❌ CA Sahab error: {str(e)}")
