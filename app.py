...
# ─────────────────────────────────────────────
# 1. PAGE CONFIG & ULTRA-PREMIUM UI/UX
# ─────────────────────────────────────────────
...
[data-testid="stSidebar"] .stTextInput > div > div > input { background: rgba(255,255,255,0.04) !important; border: 1px solid var(--border) !important; border-radius: var(--radius-xs) !important; color: var(--text) !important; font-size: 0.83rem !important; padding: 8px 12px !important; }
...
.stTabs [data-baseweb="tab"] { background: transparent !important; border-radius: 10px !important; color: var(--muted-2) !important; font-weight: 500 !important; font-size: 0.86rem !important; padding: 9px 22px !important; border: none !important; transition: var(--transition-fast) !important; }
...
.stDataFrame, [data-testid="stDataFrame"] { border-radius: var(--radius-sm) !important; border: 1px solid var(--border) !important; overflow: hidden !important; box-shadow: 0 4px 32px rgba(0,0,0,0.25) !important; }
...
.stRadio > div > label:has(input:checked) { border-color: rgba(124,111,255,0.45) !important; background: rgba(124,111,255,0.14) !important; color: #D0CBFF !important; }
    ...
    if st.session_state.scanned_data is not None:
        ...
        with st.form("edit_form"):
            ...
            if st.form_submit_button("✅ Secure Database Commit", use_container_width=True):
                final_data = {
                    "voucher_type": v_type, "vendor_name": v_name, "gst_number": v_gst,
                    "vendor_address": v_addr, "bank_details": v_bank, "invoice_date": v_date,
                    "invoice_number": v_inv_no, "base_price": v_base, "cgst_amount": v_cgst,
                    "sgst_amount": v_sgst, "igst_amount": v_igst,
...
with tab2:
    ...
    st.info("💡 Pro Tip: Speak naturally. e.g., 'Sold 10 pipes to Manoj Enterprises at 50 rupees each with 18% GST.'")
        ...
        with st.form("edit_voice_form"):
            ...
            if st.form_submit_button("✅ Commit Voice Entry", use_container_width=True):
                voice_final_data = {
                    ...
                    "invoice_date": voice_date, "gst_number": voice_gst,
                    "vendor_address": final_address, "bank_details": voice_bank,
...
with tab3:
    ...
    if total_bills > 0:
        ...
        def create_pdf_bill(bill_data):
            ...
            if cgst > 0:
                pdf.cell(155, 8, "Add: CGST", 1, 0, 'R'); pdf.cell(35, 8, f"{cgst:,.2f}", 1, 1, 'R')
            if sgst > 0:
                pdf.cell(155, 8, "Add: SGST", 1, 0, 'R'); pdf.cell(35, 8, f"{sgst:,.2f}", 1, 1, 'R')
            if igst > 0:
                pdf.cell(155, 8, "Add: IGST", 1, 0, 'R'); pdf.cell(35, 8, f"{igst:,.2f}", 1, 1, 'R')
...
with tab4:
    ...
    def generate_tally_xml(invoices_data):
        ...
        for inv in invoices_data:
            ...
            if isinstance(line_items, list):
                for itm in line_items:
                    ...
                    if item_name:
                        unique_items.add((item_name, str(itm.get('unit', 'Nos')).replace("&", "&amp;")))
        ...
        for inv in invoices_data:
            raw_date      = str(inv.get("invoice_date") or "20260409").replace("-", "")
    ...
    with ec1: st.markdown("""<div class="metric-card purple"><span class="metric-icon">🏦</span><div class="metric-label">Target Format</div><div style="color:#A89EFF;font-family:'Syne',sans-serif;font-weight:800;font-size:1.1rem;">Tally ERP 9</div></div>""", unsafe_allow_html=True)
    with ec2: st.markdown(f"""<div class="metric-card green"><span class="metric-icon">📦</span><div class="metric-label">Compiled Batch</div><div style="color:#6EE7B7;font-family:'Syne',sans-serif;font-weight:800;font-size:1.1rem;">{len(selected_invoices)} Records</div></div>""", unsafe_allow_html=True)
    with ec3: st.markdown("""<div class="metric-card amber"><span class="metric-icon">⚡</span><div class="metric-label">System Status</div><div style="color:#FCD34D;font-family:'Syne',sans-serif;font-weight:800;font-size:1.1rem;">Ready for Link</div></div>""", unsafe_allow_html=True)
